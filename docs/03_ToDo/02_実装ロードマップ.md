...
- [ ] レビュー → 修正反映(議事録改訂を受けた章書き換え:01 思想 / 03 検証 / 04 創発)
- [ ] 最終提出

## Phase 5: MBTI ペルソナ + 創発イベント実装(目標: 2026-05-03 までに完了)

> 2026-04-29 議事録の決定を反映する実装フェーズ。
> 比較軸を「集団サイズ × 創発イベント」に切り替え、ペルソナを MBTI ベースに刷新する。

### 5-1. ペルソナ刷新(MBTI 対応)

- [ ] **P5-01** [src/agent/persona.py](../../src/agent/persona.py) を新スキーマに書き換え
  - 新フィールド:`age`(20〜70)、`gender`(male/female)、`nationality`(プール抽選)、`mbti`(16 値)
  - 旧フィールド削除:`role`、`tenure_years`、`name`、`location_label`、`company_type_label`
  - 詳細:[07_ペルソナ拡張](../01_設計書/06_システム設計/04_エージェント設計/07_ペルソナ拡張.md)
- [ ] **P5-02** `Persona.to_prompt()` を 2026-04-29 議事録 §4.4 雛形に変更
  - `私は{age}代の{gender_jp}です。\n国籍: {nationality}\nMBTI: {mbti}\n日本語で回答してください。`
- [ ] **P5-03** 既存テスト([tests/test_phase3_persona.py](../../tests/test_phase3_persona.py))を新スキーマに合わせ更新

### 5-2. 創発イベント実装(プラン B)

- [ ] **P5-04** `src/events/alien.py`(宇宙人イベント)
  - 発火条件:特定 step で全 agent に通達
  - prompt 注入:「現在、地球外生命体との接触が確認されています」程度の粗い投げ込み(指示は書かない)
- [ ] **P5-05** `src/events/zero_gravity.py`(無重力イベント)
  - 発火条件:特定 step で物理法則の崩壊を通達
  - prompt 注入:「現在、無重力状態が発生しています。物理法則が崩壊しています」程度
- [ ] **P5-06** `Event` 基底クラスの抽象化(火災以外の追加に備える、旧 Stretch-B B6-01)

### 5-3. シナリオ yaml(集団サイズ × イベント)
...
- [ ] 部署分けなし:全員を 1 つの場(`main_office` のみ)に配置

### 5-4. 100 人ラン整備(粉川氏マシン側で実施想定)

- [ ] **P5-11** Qwen3 34B Uncensored への切替設定(`base.yaml` に分岐 or 別 yaml)
- [ ] **P5-12** Ollama 並列度の調整(コンテキスト超過 → モデル切替問題の回避、ターミナル直叩き)
- [ ] **P5-13** 100 体 × 100 step の完走確認(動画 + 思考可視化)

### 5-5. 提出物(PDF 再生成)

- [ ] **P5-14** [`build/chapters/`](../../build/chapters/) を新比較軸に合わせて差し替え
  - 01_思想と背景:4 象限の説明を MBTI/創発イベントに差し替え
  - 03_検証結果:5/100 比較 + 宇宙人 vs 無重力 の表
  - 04_創発観察:新ランの抜粋に差し替え。旧 4 象限結果は「副産物」として併載
- [ ] **P5-15** PDF 再生成 → 最終確定
